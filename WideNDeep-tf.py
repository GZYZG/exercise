...
from tensorflow.keras.layers import Dense, Embedding, Concatenate, Flatten, Add, Dropout, Activation
...
def build_embedding_layers(feature_columns, input_layers_dict, is_linear):
    ...
    if is_linear:
        for fc in sparse_feature_columns:
            embedding_layers_dict[fc.name] = Embedding(fc.vocabulary_size, 1, name='1d_emb_' + fc.name)  # Wide 部分的稀疏特征的 Embedding 就是对应的权重向量
    else:
        for fc in sparse_feature_columns:
...
def get_dnn_logits(dense_input_dict, sparse_input_dict, sparse_feature_columns, dnn_embedding_layers):
    ...
    dnn_input = Concatenate(axis=1)([concat_dense_inputs, concat_sparse_kd_embed])  # B x (dense_dim + sparse_dim)
...
if __name__ == "__main__":
    ...
    linear_feature_columns = [SparseFeat(feat, vocabulary_size=data[feat].nunique(), embedding_dim=4) for feat in sparse_features] + \
                             [DenseFeat(feat, 1, ) for feat in dense_features]

    dnn_feature_columns = [SparseFeat(feat, vocabulary_size=data[feat].nunique(), embedding_dim=4) for feat in sparse_features] + \
                          [DenseFeat(feat, 1, ) for feat in dense_features]

    print(f"dense_features: {dense_features}\nsparse_features: {sparse_features}")
